...
prenode1 <- function(){
    ...
    title1 <- c("CHR","POS","REF","ALT","Subject","parent_min","esp6500si_all","Gene","Func.refgene","ExonicFunc.refgene","AAChange.refgene","X1000g2014oct_all","SIFT_score","Polyphen2_HDIV_score","MutationTaster_score","GERP.._RS")            
    title2 <- c("CHROM","POS","REF","ALT","proband.ID.GT.AD.DP.GQ.PL.","Inherited.from","esp.MAF","GeneName","VarFunc","VarClass","AAChange","X1KGfreq","SIFTscr","PP2scr","MutTscr","GERP") 
    ...
    prenode0(samples,trun,mis3,genes,mutrate,str="New")
...
#     library(MASS)
#     pa <- fitdistr(tmp,"exponential")
#     tmp <- pexp(tmp, rate = pa$estimate[1], lower.tail = TRUE, log.p = FALSE)
    tmp
}
...
prenode2 <- function(strscore,samples,trun,mis3,mutrate=0,str,flag=1){
