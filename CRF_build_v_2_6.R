...
#     select <- rep(TRUE,net$size)
#     net.dist <- get_shortest_distances(net$matrix, select)
#     net.dist[net.dist == -1] <- Inf
#     net.dist[cbind(1:net$size, 1:net$size)] <- Inf  #????
#     W <- 1 / net.dist    
...
#         W1[,1] <- c(1/deg[n2],1/(2*deg[n2])) ## when deg == 1, it will be more easy to highly rank
#         W2[,1] <- c(1/deg[n1],1/(2*deg[n1]))
#         W1[,1] <- c(1/(deg[n2]),1/(2*deg[n2]))
#         W2[,1] <- c(1/(deg[n1]),1/(2*deg[n1]))
#         W1[,2] <- 1-W1[,1]
#         W2[,2] <- 1-W2[,1]
#         W <- pmax(W1 * query.net[n1, n2], t(W2) * query.net[n2, n1])
        #W <- sweep(W, 1, rowSums(W), FUN="/")
        #crf$edge.pot[[e]] <- (S1 + S2) *  W/ 2
        #crf$edge.pot[[e]] <- (S1 + S2) *  pmax(W1 * query.net[n1, n2], t(W2) * query.net[n2, n1])/ 2

        #S[,1] <- c(1-abs(S1[1,1]-S2[1,1]),abs(S1[2,1]-S2[1,2]))
        #S[,1] <- c(1/(abs(S1[1,1]-S2[1,1])+1), 1 - 1/(abs(S1[2,1]-S2[1,2])+1))
        #S[,2] <- 1 - S[,1]
#         if(S1[1,1] >= 0.5 & S2[1,1] >= 0.5){
#             riskf <- 2*(sqrt(S1[1,1]*S2[1,1])-0.5)+1  # 16 ~~ 0.94/0.06
#             c <- min(riskf , sqrt(deg[n1]*deg[n2])/2)
#         }else{
#             c <- 1
#         }
        c <- 0.5*sqrt(S1[1,1]*S2[1,1]) # -log(0.63*abs(S1[1,1]-S2[1,1])+0.37)
#         if((S1[1,1]-0.5)*(S2[1,1] - 0.5) < 0){
#             c <- min(S1[1,1],S2[1,1])^2
#         }
#         S[1,1] <- 0.5+(1-abs(S1[1,1]-S2[1,1]))/sqrt(deg[n1]*deg[n2]); S[2,1] <- 1-S[1,1];
#         S[2,2] <- 0.5+(1-abs(S1[2,2]-S2[2,2]))/sqrt(deg[n1]*deg[n2]); S[1,2] <- 1-S[2,2];
        S[1,1] <- 0.5+c*(1-abs(S1[1,1]-S2[1,1]))/sqrt(deg[n1]*deg[n2]); S[2,1] <- 1-S[1,1];
        S[2,2] <- 0.5+c*(1-abs(S1[2,2]-S2[2,2]))/sqrt(deg[n1]*deg[n2]); S[1,2] <- 1-S[2,2];
        crf$edge.pot[[e]] <- S1*S*S2
#         S[1,1] <- 0.5+1/sqrt(deg[n1]*deg[n2]); S[2,1] <- 1-S[1,1];
#         S[2,2] <- 0.5+1/sqrt(deg[n1]*deg[n2]); S[1,2] <- 1-S[2,2];
        #S <- matrix(c(0.1,0.1,0.9,0.9),2,2)
        #crf$edge.pot[[e]] <- pmin(S1,S2) / abs(S1-S2)
        #crf$edge.pot[[e]] <- (S1+S2)/2
        #crf$edge.pot[[e]] <- 2*S1*S2/ (S1+S2)
        #crf$edge.pot[[e]] <- 2*pmax(S1*S2,t(S2)*t(S1))/(S1+S2)  
        #crf$edge.pot[[e]] <- W * S 
        #crf$edge.pot[[e]] <- pmax(S1,S2) * W
        #crf$edge.pot[[e]] <- pmin(S1,S2) * W
        #crf$edge.pot[[e]] <- W
        #crf$edge.pot[[e]] <- pmin(W1 * query.net[n1, n2], t(W2) * query.net[n2, n1])


        #S <- matrix(c(0.1,0.1,0.9,0.9),2,2)
        #crf$edge.pot[[e]] <-  W*S / (deg[n1]*deg[n2])
        #crf$edge.pot[[e]] <-  (deg[n1] + deg[n2]) * S * (1/(deg[n1]^2*deg[n2]) + 1/(deg[n2]^2+deg[n1])) 

        #crf$edge.pot[[e]] <- pmax(W1 * query.net[n1, n2], t(W2) * query.net[n2, n1])
        #crf$edge.pot[[e]] <- pmin(W1 * query.net[n1, n2], t(W2) * query.net[n2, n1])
        #crf$edge.pot[[e]] <- (S1 + S2) *  pmin(W1 * query.net[n1, n2], t(W2) * query.net[n2, n1])/ 2
        #crf$edge.pot[[e]] <- (S1 + S2) *  pmax(W1,t(W2))/ 2
        #crf$edge.pot[[e]] <- (S1 + S2) *  pmin(W1,t(W2))/ 2
