...
build_model <- function(modnodesim,net,bflag=1){   
    ...
    for (e in 1:crf$n.edges){
        ...
        #c <- min(exp((sqrt(S1[1,1]*S2[1,1])-0.5)/(1-S1[1,1]*S2[1,1]))/(sqrt(deg[n1]*deg[n2])), 0.5)
        #c <- min(exp((S1[1,1]*S2[1,1]-0.25)/(1-S1[1,1]*S2[1,1]))/(exp(1)*sqrt(deg[n1]*deg[n2])), 0.5)
        ...
        #c <- min((1/(1-sqrt(S1[1,1]*S2[1,1])))*exp(S1[1,1]*S2[1,1]-0.25)/(exp(2)*sqrt(deg[n1]*deg[n2])), 0.5)
        #c <- min(exp(S1[1,1]*S2[1,1]-0.25)/sqrt(deg[n1]*deg[n2]), 0.5)
        #c <- min(plogis(S1[1,1]*S2[1,1],location=S1[1,1]*S2[1,1],scale=2/(1-S1[1,1]*S2[1,1])),0.5)
        #c <- min(exp(sign(min(S1[1,1],S2[1,1])-0.5)/abs(S1[1,1]*S2[1,1]-0.5))/(sqrt(deg[n1]*deg[n2])*exp(S1[1,1]*S2[1,1]-0.5)),0.5)
        #c <- min(exp((sqrt(S1[1,1]*S2[1,1])-0.5))/sqrt(deg[n1]*deg[n2]),0.5)
#         c <- min(exp(abs(min(S1[1,1],S2[1,1])-1)/abs(S1[1,1]*S2[1,1]-sqrt(S1[1,1]*S2[1,1])))/(sqrt(deg[n1]*deg[n2])*exp(S1[1,1]*S2[1,1]-1)),0.5)
#         W[1,1] <- 0.5+c; W[2,1] <- 1-S[1,1];
#         W[2,2] <- 0.5+c; W[1,2] <- 1-S[2,2];
        #crf$edge.pot[[e]] <- S1*W*S2
#         W[1,1] <- 1/(1-log(1-abs(S1[1,1]-S2[1,1])))
#         W[1,2] <- 0
#         W[2,1] <- 0
#         if(min(S1[1,1],S2[1,1])< 0.5){
#             W[2,2] <- W[1,1]*(S1[1,1]+S2[1,1])/(2-S1[1,1]-S2[1,1])
#         }else{
#             W[2,2] <- sqrt(S1[1,1]*S2[1,1])*W[1,1]*(S1[1,1]+S2[1,1])/(2-S1[1,1]-S2[1,1])
#         }
#         crf$edge.pot[[e]] <- (0.5*(S1+S2)*W)
