...
#             }
    ...
    pdf(file="plot/leaveone_8_16.pdf",width=10,height=10)
    par(mai=c(4,2,2,2))
...
Meta <- function(){
   ...
   for(i in 1:5){
    ...
    ##--------------------------------------------------------------------
    ## NDD repeat result
    rep_NDD <- read.csv("DDD_repeat/TADAdenovo_rep_NDD_dmis_7_29.csv")
    genes <- rep_NDD[rep_NDD[,"qvalue.dn"] <= 0.05,1]
    genes1 <- rep_NDD[rowSums(rep_NDD[,c("dn.LoF","dn.mis3")]) > 0,1]
    nm1 <- rep(0,5)
    nm2 <- rep(0,5)
    for(i in 1:length(geneT)){
        nm1[i] <- length(intersect(geneT[[i]],genes))
        nm2[i] <- length(intersect(geneT[[i]],genes1))
    }
