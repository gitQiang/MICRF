...
batch_poisson_hq <- function(){
    ...
    tmp[,"p_value"] <- pV 
    tmp[,"adjust_p"] <- 1
    subs <- tmp[,"dn.LoF"]+ tmp[,"dn.mis3"] > 0
    tmp[subs,"adjust_p"] <- p.adjust(tmp[subs,"p_value"],n=sum(subs),method="fdr")
...
leaveone_recurrent <- function(){
    ...
    ## DAWN method 
    ### control data result  
    cfile <- "result/control/DAWN/DAWNcontrol.csv"
    ctmp <- read.csv(cfile)
    ...
    oneg <- c()
    ###  MAGI method
    cfile <- "result/control/MAGI/RandomGeneList.1"
    ...
    ### hotnet2 method 
    for(j in 1:5){
        cfile <- paste("result/control/hotnet/hotnetresult1control",netnum[j],".txt",sep="")
        ctmp <- read.table(cfile)
    ...
    pdf(file="plot/leaveone.pdf",width=10,height=10)
    par(mai=c(4,2,2,2))
    ...
    ords <- c(1,2,3,4,9,7,12,5,10,8,13,6,11)
    #cols <- c("red","green","blue","orange","orange","yellow","yellow","pink","pink","cyan","cyan","purple","purple")
    
    barplot(t(nLeo)[,ords], names.arg=legend[ords], beside=T, ylab="#gold risk genes", 
            cex.names=1.5, las=2, ylim=c(0,max(nLeo)), col=c("red","darkblue"),cex.lab=1.5,cex.axis=1.25)
    #box(bty="l")
    ...
    pdf(file="plot/missing.pdf",width=10,height=7)
    a1 <- max(deV[netex$node %in% Tset])
    a2 <- max(beV[beV[,1] %in% Tset,2])
    nulist <- list()
    nulist[[1]] <- log10(deV[netex$node %in% oneg])
    nulist[[2]] <- log10(deV[netex$node %in% setdiff(Tset,oneg)])
    ...
    par(mfrow=c(1,2))
    boxplot(nulist,names=c("recovered","non-recovered"),main = "Gene interactions", xlab = "", ylab = "Degree (log10)", ylim=c(0.5,1.7))
    text(1.5,1.65,paste("t test pvalue: ",format(t.test(nulist[[1]],nulist[[2]])$p.value,digits=3),sep=""))
    boxplot(nu1list,names=c("recovered","non-recovered"),main= "Gene Betweenness", xlab="", ylab="Betweenness (log10)")
    text(1.5,0.9,paste("t test pvalue: ", format(t.test(nu1list[[1]],nu1list[[2]])$p.value,digits=3), sep=""))
    dev.off()   
    ...
    oneg <- RWRg
    a = sum(rowSums(metam[metam[,1] %in% oneg,2:3])>0)
    b = sum(rowSums(metam[metam[,1] %in% setdiff(Tset,oneg),2:3])>0)
    
    a <- matrix(c(a,length(oneg),b,79-length(oneg)),2,2)
    fisher.test(t(a))
...
Meta <- function(){
    ...
    source("enrichana_6_12.R")
    ###  gold risk genes
    Tset <- unlist(read.table("DDD_mutations/nature14135-s2/Genes.txt"))
        
    filenames <- c("ASD/TADAresult/TADAdenovo_Meta.csv","result/Nat1377_meta/DAWN/DAWN_Meta.csv","result/Nat1377_meta/MAGI/RandomGeneList.3","result/Nat1377_meta/RWR/coexp1/hotnetresult1Meta_7_321.txt","result/Nat1377_meta/MICRF/v4/coexp/CRFresult_0.2Meta_7_3LBP_7.txt")
    ...
    gp <- read.csv(poifile)
    gp[is.na(gp[,"min_p"]),"min_p"] <- 1
    gp[,"adjust_p"] <- 1
    gp[is.na(gp[,"dn.LoF"]),"dn.LoF"] <- 0
    gp[is.na(gp[,"dn.mis3"]),"dn.mis3"] <- 0
    subs <- gp[,"dn.LoF"] + gp[,"dn.mis3"] >0
    gp[subs,"adjust_p"] <- p.adjust(gp[subs,"min_p"],n=sum(subs)*3,method="fdr")
    ...
    ## show in Cytoscape
    source("~/.Rprofile")
    qwt(geneT[[5]],file="result/Nat1377_meta/MICRF_meta.txt")
    
    
    input <- read.csv("ASD/TADAresult/TADAdenovo_Meta.csv")
    tmp <- cbind(input[,1],"=",-log10(input[,"pval.TADA.dn"]))
...
comple <- function(){
    ...
    filenames <- c("ASD/TADAresult/TADAdenovo_Meta.csv","result/Nat1377_meta/DAWN/DAWN_Meta.csv","result/Nat1377_meta/MAGI/RandomGeneList.3","result/Nat1377_meta/RWR/coexp1/hotnetresult1Meta_7_321.txt","result/Nat1377_meta/RWR/HPRD/hotnetresult1Meta_7_320.txt","result/Nat1377_meta/RWR/STRING/hotnetresult1Meta_7_33.txt","result/Nat1377_meta/RWR/iRef/hotnetresult1Meta_7_36.txt","result/Nat1377_meta/RWR/coexp/hotnetresult1Meta_7_37.txt","result/Nat1377_meta/MICRF/v4/coexp/CRFresult_0.2Meta_7_3LBP_7.txt") 
    ...
    b <- b[!duplicated(b[,1]),]
    noncov <- b[b[,"qvalue.dn"]<0.3,1] # 0.1 133; 0.2 186; 0.5 656; 0.3 269
    ...
    #tft <- read.delim("../shares/ENCODE_TF_ChIP-seq_2015.txt",header=FALSE)
    tft <- read.delim("../shares/ChEA.txt",header=FALSE)
    #tft <- read.delim("../shares/TRANSFAC_and_JASPAR_PWMs.txt",header=FALSE)
    #tfs <- sapply(1:dim(tft)[1], function(i) unlist(strsplit(tft[i,1],"_"))[1])
    #sum(noncov %in% toupper(tfs))
    
    n <- dim(tft)[2]
    
    nt <- 1:dim(tft)[1]
    for( i in 1:dim(tft)[1]){
        nt[i] <- sum(noncov %in% tft[i,3:n]) 
    }
    ...
    tars <- c()
    for( i in 3:dim(tft)[2]){
        tars <- union(tars,tft[,i])
    }
    tars <- setdiff(tars,"")
    sum(noncov %in% tars)
    
    files <- list.files("../human/human_tf_regulated_gene/",full.names = TRUE)
    tfs <- c()
    tars <- c()
    for(i in 1:length(files)){
        tmp <- read.table(files[i])
        tars <- union(tars,tmp[,2])
        tfs <- union(tfs,tmp[,1])
    }
    sum(noncov %in% tars)
    sum(noncov %in% tfs)
